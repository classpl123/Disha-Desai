...
# reading the files -
path="F:\disha Imarticus Files/salesdata.csv"
sales = pd.read_csv(path)
sales.head(5)
train = sales.loc[0:8524,:]
test =  sales.loc[8525:14205,:]
...
def modelfit(algorithm, xtrain, xtest, predictors, target, IDcolumn, filename):
    ...
    cv_score = cross_validation.cross_val_score(algorithm, xtrain[predictors], xtrain[target], cv=20, scoring='mean_squared_error')
    cv_score = np.sqrt(np.abs(cv_score))
    
    #Print model report:
    print "\nModel Report"
    print "RMSE : %.4g" % np.sqrt(metrics.mean_squared_error(xtrain[target].values, xtrain_predictions))
